...
exports.PosModel = Backbone.Model.extend({
    ...
    models: [
    {
        ...
        fields: ['name','street','city','state_id','country_id','vat',
                 'phone','zip','mobile','email','barcode','write_date','age','gander_status','personal_number','beneficiary_number',
                 'property_account_position_id','property_product_pricelist','dasb','parent_id'],
        domain: [['customer','=',true]],
        loaded: function(self,partners){
            self.partners = partners;
            var dasb_id = self.config.dasb_id[0] || false;
            self.db.add_partners(partners, dasb_id);
        },
    },{
        model:  'res.users',
    ...
    load_new_partners: function(){
        ...
        var dasb_id = parseInt($("span.dasb").text());

        var fields = _.find(this.models,function(model){ return model.model === 'res.partner'; }).fields;
        var domain = [['customer','=',true],['dasb','=', dasb_id || false],['write_date','>',this.db.get_partner_write_date()]];
        rpc.query({
                model: 'res.partner',
                method: 'search_read',
                args: [domain, fields],
            ...
            .then(function(partners){
                if (self.db.add_partners(partners, dasb_id)) {   // check if the partners we got were real updates
                    def.resolve();
                } else {
                    def.reject();
                }
