...
var PosDB = core.Class.extend({
    ...
    _partner_search_string: function(partner){
        ...
        if(partner.personal_number){
            str += '|' + partner.personal_number.split(' ').join('');
        }
        if(partner.beneficiary_number){
            str += '|' + partner.beneficiary_number.split(' ').join('');
        }
        str = '' + partner.id + ':' + str.replace(':','') + '\n';
        return str;
    },
    add_partners: function(partners, dasb_id){
        var updated_count = 0;
        var new_write_date = '';
        var partner;
        ...
        for(var i = 0, len = partners.length; i < len; i++){
            ...
            if(dasb_id && partner.dasb[0] == dasb_id)
            {
