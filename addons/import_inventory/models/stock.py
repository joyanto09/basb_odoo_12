                ...
                if self.location_id_option == True : 
                    if i!= 0:
                        ...
                        if bool(val):
                            ...
                            stock_line_id = stock_line_obj.sudo().create({
                                'product_id' : int(val['product']),
                                'location_id' : stock_location_id.id,
                                'product_uom_id' : product_uom_id.id,
                                'product_qty' : int(val['quantity']),
                                'inventory_id' : inventory_id.id,
                                })

                            stock_line_id._onchange_quantity_context()
                            stock_line_id.sudo().write({'product_qty' : int(val['quantity'])})
                    ...
                    if row_no <= 0:
                        ...
                        if line:
                            ...
                            if bool(val):
                                ...
                                stock_line_id._onchange_quantity_context()
                                stock_line_id.sudo().write({'product_qty' : val['quantity']})
